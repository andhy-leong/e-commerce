...
switch ($action) {
    ...
    // --- ROUTE MISE À JOUR POUR LE FORMULAIRE ---
    case 'modifierProduitForm':
        $produitId = $_GET['id'] ?? null;
        if ($produitId) {
            $produitController->afficherFormulaireModification($produitId);
        } else {
            echo "Erreur : ID de produit non spécifié.";
        }
        break;

    // --- ROUTE MISE À JOUR POUR TRAITER L'UPLOAD ---
    case 'modifierProduit':
        $produitId = $_POST['id'] ?? null; // L'ID vient du champ caché du formulaire
        
        if ($produitId) {
            // Le contrôleur gère lui-même $_POST et $_FILES
            $produitController->modifierProduit($produitId); 
        } else {
            echo "Erreur : ID de produit non spécifié lors de la soumission.";
        }
        break;
